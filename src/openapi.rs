...
pub fn build_spec(
    ...
    base_path: &str,
    package_models: &std::collections::HashMap<String, ResolvedModel>,
) -> OpenApi {
...
/// GET /spec — return OpenAPI JSON for entity APIs (default model + all package models).
pub async fn spec_handler(State(state): State<AppState>) -> Json<OpenApi> {
    let model = state.model.read().expect("model read lock");
    let package_models = state.package_models.read().expect("package_models read lock");
    let spec = build_spec(&model, "/api/v1", &package_models);
    Json(spec)
}
