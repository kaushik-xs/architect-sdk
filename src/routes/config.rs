//! Config ingestion routes: POST and GET per config kind, plus plugin install.
...
use crate::handlers::plugin::install_plugin;
use crate::state::AppState;
use axum::{routing::post, Router};

pub fn config_routes(state: AppState) -> Router {
    Router::new()
        .route("/config/plugin", post(install_plugin))
